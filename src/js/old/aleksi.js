...
function getFullWord(event) {
  ...
  var context = textNode.parentNode.textContent;
...
function configure_dialog() {
    ...
    var aleksi_div = jquery_aleksi('#aleksi');
    aleksi_div.tabs();
...
function set_anki_connect_get_decks_link() { 
    ...
    var anki_connect_get_decks_button_container = jquery_aleksi("#anki_connect_get_decks_button_container");
...
function update_translations_table(result) {
    var progress_indicator = jquery_aleksi("#aleksi_analysis_progress_indicator");
...
jquery_aleksi(document).ready(async function() {
  if (mode == 'app') {
    set_menu_placement();
  }
  var page_overlay = jquery_aleksi('<div id="aleksi_overlay"><img src="'+await get_setting('loading_spinner_url')+'"/> <p id="aleksi_overlay_msg">Processing</p></div>');
